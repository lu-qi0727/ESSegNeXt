...
@TRANSFORMS.register_module()
class LoadMultipleAnnotations(MMCV_LoadAnnotations):
    """Load annotations for semantic segmentation provided by dataset.
    ...
    def _load_seg_map(self, results: dict) -> None:
        ...
        img_bytes2 = fileio.get(
            results['img_path2'], backend_args=self.backend_args)
        gt_semantic_seg2 = mmcv.imfrombytes(
            img_bytes2, flag='unchanged',
            backend=self.imdecode_backend).squeeze().astype(np.uint8)
        ...
        if self.reduce_zero_label:
            ...
            # gt_semantic_seg = gt_semantic_seg - 1
            # gt_semantic_seg[gt_semantic_seg == 254] = 255
            gt_semantic_seg2[gt_semantic_seg2 != 0] = 1
        # modify if custom classes
        if results.get('label_map', None) is not None:
            # Add deep copy to solve bug of repeatedly
            # replace `gt_semantic_seg`, which is reported in
            ...
            for old_id, new_id in results['label_map'].items():
                gt_semantic_seg[gt_semantic_seg_copy == old_id] = new_id
        results['gt_seg_map'] = gt_semantic_seg
        results['gt_seg_map2'] = gt_semantic_seg2
        results['seg_fields'].append('gt_seg_map')
